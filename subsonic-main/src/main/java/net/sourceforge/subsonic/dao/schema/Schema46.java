...
public class Schema46 extends Schema {
    ...
    public void execute(JdbcTemplate template) {
        ...
        if (!tableExists(template, "transcoding2")) {
            ...
            template.execute("insert into transcoding2(name, source_formats, target_format, step1) values('ogg audio'," +
                    "'mp3 aac m4a flac wav wma aif aiff ape mpc shn', 'ogg', " +
                    "'ffmpeg -i %s -ab %bk -v 0 -acodec libvorbis -f ogg -')");

            template.execute("insert into transcoding2(name, source_formats, target_format, step1) values('webm audio'," +
                    "'mp3 ogg oga aac m4a flac wav wma aif aiff ape mpc shn', 'webm', " +
                    "'ffmpeg -i %s -ab %bk -v 0 -acodec libvorbis -f webm -')");
